...
def generate_user_trustscore(n_user: int, ranges: list[tuple[float, float]], distributions: list[float]):
    user_id = range(0, n_user)
    weights = [int(x * n_user) for x in distributions]
    identity_trust = generate_random_float(ranges, weights)
    behavior_trust = generate_random_float(ranges, weights)
    data = data = {"user": user_id, "identity_trust": identity_trust, "behavior_trust": behavior_trust}
    return pd.DataFrame.from_dict(data)


def generate_organization_trustscore(n_org: int, ranges: list[tuple[float, float]], distributions: list[float]):
